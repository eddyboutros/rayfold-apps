...
const GUIDE: Group[] = [
  ...
  },
  {
    ...
    entries: [
      { what: "A page with no Rayfold client", how: "The shell reads its rail from GET /api/workspace/projects, a query bound to a path: the same policy and the same default view, answered as plain JSON with the session cookie a browser already sends.", where: "Project settings on a project's page: rename it or change its colour, and the rail follows. Its default assignee takes every new issue that names nobody.", go: { view: "project", label: "Project settings" }, file: "web/shell/src/app/app.ts (loadProjects)" },
  ...
  },
  {
    ...
    entries: [
      ...
      { what: "A link across services", how: "An issue's attachment is a document that lives in the documents service. The workspace keeps the link; a rename over there reaches it over the relay, like the feed.", where: "Open an issue, Attach a document; then rename that file under Documents and watch the pin follow.", go: { view: "project", label: "Issues" }, file: "services/workspace/src/main.ts (DocumentChanged)" },
      { what: "Partial fields (@partial)", how: "A field that may fail on its own: someone it is refused to gets the rest of the answer, with that one field null and its refusal beside it in the frame, instead of the whole operation failing.", where: "Open a product in the Catalogue. Noor, on the product team, sees its margin; everyone else sees the product with the margin withheld.", go: { view: "catalogue", label: "Catalogue" }, file: "services/catalogue/src/catalogue.rayfold (Product.cost)" },
